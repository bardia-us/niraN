...
class NativeSettings {
  const NativeSettings({
    ...
    this.localListenAddress = '0.0.0.0',
    this.realPingConcurrency = 16,
    this.realDelayUrl = 'https://www.gstatic.com/generate_204',
    this.realDelayTimeoutSeconds = 8,
  ...
  factory NativeSettings.fromMap(Map<dynamic, dynamic> map) => NativeSettings(
    connectionMode: '${map['connectionMode'] ?? 'proxy'}',
    systemProxyEnabled: map['systemProxyEnabled'] != false,
    systemProxyState:
        '${map['systemProxyState'] ?? (map['systemProxyEnabled'] == true ? 'niran' : 'clear')}',
    tunEnabled:
        map['tunEnabled'] == true ||
    ...
    allowLanConnections: map['allowLanConnections'] == true,
    localListenAddress: '${map['localListenAddress'] ?? '0.0.0.0'}',
    realPingConcurrency: _int(map['realPingConcurrency']) ?? 16,
    realDelayUrl:
        '${map['realDelayUrl'] ?? 'https://www.gstatic.com/generate_204'}',
    realDelayTimeoutSeconds: _int(map['realDelayTimeoutSeconds']) ?? 8,
  ...
  final String localListenAddress;
  final int realPingConcurrency;
  final String realDelayUrl;
  final int realDelayTimeoutSeconds;
  ...
  bool get isTunEnabled => tunEnabled || connectionMode == 'vpn';
  ...
  NativeSettings withUpdates(Map<String, Object?> values) {
    ...
    return NativeSettings(
      ...
      systemProxyEnabled: boolValue('systemProxyEnabled', systemProxyEnabled),
      ...
      enableUdp: boolValue('enableUdp', enableUdp),
      allowLanConnections: boolValue(
        'allowLanConnections',
        allowLanConnections,
      ),
      localListenAddress: stringValue('localListenAddress', localListenAddress),
      realPingConcurrency: intValue('realPingConcurrency', realPingConcurrency),
      realDelayUrl: stringValue('realDelayUrl', realDelayUrl),
      realDelayTimeoutSeconds: intValue(
        'realDelayTimeoutSeconds',
        realDelayTimeoutSeconds,
      ),
      ...
      showRecentLogsOnHome: boolValue(
