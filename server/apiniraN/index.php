...
if ($authenticated) {
    try {
        $rows = $database
            ->query('SELECT device_name, windows_username, windows_version, app_version, first_seen, last_seen FROM installations ORDER BY last_seen DESC LIMIT 1000')
            ->fetchAll();
    } catch (Throwable $exception) {
...
<head>
  ...
  <title>niraN Device Registry</title>
  <style nonce="<?= e($nonce) ?>">
  ...
  <section class="panel">
    ...
    <?php else: ?>
      ...
      <div class="table-wrap">
        <table>
          <thead><tr><th>Device Name</th><th>Windows User</th><th>Windows</th><th>niraN Version</th><th class="time">First Seen</th><th class="time">Last Seen</th></tr></thead>
          <tbody>
          <?php foreach ($rows as $row): ?>
            <tr>
              <td><?= e((string)($row['device_name'] !== '' ? $row['device_name'] : '—')) ?></td>
              <td><?= e((string)($row['windows_username'] !== '' ? $row['windows_username'] : '—')) ?></td>
              <td><?= e((string)$row['windows_version']) ?></td>
              <td><?= e((string)$row['app_version']) ?></td>
